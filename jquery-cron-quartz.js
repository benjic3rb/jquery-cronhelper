...
//                    dom="?";
                ...
                case 'Yearly':
                    ...
                    if($selector.find("input[name=yearlyType][value=byDay]").is(":checked")){
                        ...
                        let start = 1;
                        let end = 7;
                        dow=$selector.find("select.cron-yearly-day-of-week").val() // a convertir
                        switch(dow) {
                            case "SUN": 
                                dow = 0;
                                break;
                        ...
                        // Jour du mois : le 1er est compris entre
                        // le 1-7 du mois, le second 7-14, ainsi de suite
                        let dow_number = $selector.find("select.cron-yearly-nth-day").val();
                        start = (dow_number == 1) ? 1:(dow_number-1)*7;
                        dom = start + '-' + dow_number * end;
